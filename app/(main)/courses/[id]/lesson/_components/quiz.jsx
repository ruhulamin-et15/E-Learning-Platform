...
export const Quiz = ({ quizSets, courseId, isQuizComplete, report }) => {
  const [open, setOpen] = useState(false);
  const quizzes = quizSets.quizIds.map((quiz) => {
    return {
  ...
  const allQuizzes = report?.quizAssessment?.assessments || [];
  const totalQuizes = quizSets.quizIds ? quizSets.quizIds.length : 0;
  const quizzesTaken = allQuizzes.filter((quiz) => quiz.attmpted);
      ...
      <div class="max-w-[270px] bg-white border border-border rounded-md dark:bg-gray-800 dark:border-gray-700 overflow-hidden">
        ...
        <div class="p-4">
          ...
          <div className="flex items-center justify-between gap-6 text-sm mb-2 font-medium text-gray-700">
            ...
            <Badge className="bg-success/20 text-primary hover:bg-success/20">
              {quizSets.quizIds ? quizSets.quizIds.length * 5 : 0}
            </Badge>
          </div>
          <div className="flex items-center justify-between gap-6 text-sm mb-2 font-medium text-gray-700">
          ...
          >
            ...
            <span>
              {isQuizComplete ? "Quiz Taken" : "Participate in a Quiz"}
            </span>
          </Button>
        </div>
      </div>
      <QuizModal
        quizSetId={quizSets._id.toString()}
        courseId={courseId}
        quizzes={quizzes}
        open={open}
        setOpen={setOpen}
